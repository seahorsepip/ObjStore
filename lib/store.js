...
Store.prototype.fields = {
    add: async function (fields, callback = noop) {
        try {
            if (!Array.isArray(fields)) fields = [fields];
            fields.map(field => field = new Field(field));
            fields.forEach(field =>  priv.get(this).fields[field.name] = field);
            callback(null, this);
            return this;
        } catch (error) {
            callback(error);
    ...
    remove: async function (names, callback = noop) {
        try {
            if (!Array.isArray(names)) names = [names];
            names.forEach(name => delete priv.get(this).fields[name]);
            callback(null, this);
            return this;
        } catch (error) {
            callback(error);
